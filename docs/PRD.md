...
### 1.1 产品定位
面向个人大学生的一站式日程管理工具，Web 应用，以 AI 降低课表录入和维护成本，以实时同步提升使用效率。

### 1.2 核心价值
- **AI 零门槛录入**：复制教务系统 HTML → AI 解析自动建课表；自然语言一句话增删改课程/待办
- **多端同步**：服务器统一存储，浏览器随时随地访问，数据一致
- **iPad 小组件**：用 WebView 组件加载只读展示页，桌面即可查看课程与待办
...
| AI | DeepSeek API | 结构化输出 JSON Mode + SSE 流式 |
| 脚本沙箱 | Mozilla Rhino | 自定义循环待办规则 |
...
| P2.9 iPad 小组件 | WebView 加载 `/widget` 展示页 + RESTful 接口 | 已实现 |
...
|------|----------|
| v0.1 | 初稿 |
| v0.2 | 确定技术选型 (React + MyBatis-Plus)；API Key 安全方案；AI Prompt 补充日历上下文；更新作息时间表为实际数值；清理已确认项 |
| v0.3 | 细化 AI 与同步方案；移除 AI Prompt 中作息时间表；全部待定项已确认 |
